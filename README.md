...
A photo enhancement product built for VIP25, now full-stack:

- **Frontend** (`index.html`) — sharpens, denoises, and corrects
  contrast using classical processing, plus two free AI models that run
  entirely in the browser (MAXIM deblurring, ESRGAN upscaling). No
  server required for any of this tier — everything above runs
  client-side, nothing uploaded anywhere.
- **Backend** (`/backend`) — a real, self-hosted server running actual
  **GFPGAN** + **Real-ESRGAN** for guaranteed, production-grade face
  restoration, for when the free client-side tier hits its ceiling on
  a badly degraded source. Optional — the frontend works standalone
  without it.
...
```
clarity-image-enhancer/
├── index.html          ← the frontend (HTML + CSS + JS, single file)
├── docker-compose.yml  ← runs frontend + backend together for local testing
├── backend/
│   ├── app.py           ← FastAPI server (GFPGAN + Real-ESRGAN)
│   ├── requirements.txt
│   ├── Dockerfile
│   └── README.md         ← backend-specific deployment guide, read this before deploying
└── README.md            ← this file
```

The frontend has no build step, no dependencies, no `npm install` — it's
plain HTML/CSS/JavaScript using the Canvas API. The backend is Python/
Docker; see `backend/README.md` for running and deploying it.
